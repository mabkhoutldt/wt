...
bool ProcessImpl::createAuthnRequest(Wt::Http::Response &response)
{
  ...
  std::unique_ptr<saml2md::SingleSignOnService> endpoint(saml2md::SingleSignOnServiceBuilder::buildSingleSignOnService());
  endpoint->setBinding(redirectBinding.get());
  endpoint->setLocation(location.c_str());
